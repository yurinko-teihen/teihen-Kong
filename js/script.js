...
let trackRight    = 0,        // 右向きアニメーションの行インデックス
    ...
    ismariostar   = false,    // マリオがスターを持っているか
...
let gameLoop = () => {
  gameclearance = setInterval(() => {
    ...
    for (let eachbluebarrel of barrelArraynext) {
      ...
      eachbluebarrel.draw();
      if (collisionDetectionBlue(eachbluebarrel) && !ismariostar) {
        if (!ismarioalive) {
          setTimeout(() => {
    ...
    for (let eachbarrelladder of barrelArrayLadder) {
      ...
      if (collisionDetection(eachbarrelladder) && !ismariostar) {
        if (!ismarioalive) {
          setTimeout(() => {
            clearInterval(gameclearance);
          }, COLLISION_STOP_DELAY);
...
let starCollision = (eachstar) => {
  let index = 0;
  const starW = single_width  * SPRITE_SCALE;
  const starH = single_height * SPRITE_SCALE;

  if (marioPlayer.positionX < eachstar.positionX + starW &&
      marioPlayer.positionX + single_width > eachstar.positionX &&
      marioPlayer.positionY < eachstar.positionY + starH &&
      marioPlayer.positionY + single_height > eachstar.positionY) {
    // スターを配列から削除し、スター状態をONにする
    index = starArray.indexOf(eachstar);
...
let starTimerId = null; // スター効果タイマーのハンドル（リセット用）
...
let afterCollision = () => {
  ...
  // RESPAWN_DELAY ms後にゲームオーバー画面へ遷移し、スターをリセット
  setTimeout(() => {
    stopGameCanvas();
    resetStars();
  }, RESPAWN_DELAY);
...
let afterGameWon = () => {
  ...
  // GAME_WON_DELAY ms後にクリア画面へ遷移し、スターをリセット
  setTimeout(() => {
    gameWonCanvas();
    resetStars();
  }, GAME_WON_DELAY);
...
// スター配列を初期状態（3個）に戻すユーティリティ
let resetStars = () => {
  starArray = [
    new STAR(HAMMER_ONE_X,   HAMMER_ONE_Y),
    new STAR(HAMMER_TWO_X,   HAMMER_TWO_Y),
    new STAR(HAMMER_THREE_X, HAMMER_THREE_Y)
  ];
}
